...
static void get_all_cpustat(struct kernel_cpustat *cpu_stat)
{
	...
	for_each_possible_cpu(cpu) {
		...
		cpu_stat->cpustat[CPUTIME_USER]		+= cpustat[CPUTIME_USER];
		cpu_stat->cpustat[CPUTIME_NICE]		+= cpustat[CPUTIME_NICE];
		cpu_stat->cpustat[CPUTIME_SYSTEM]	+= cpustat[CPUTIME_SYSTEM];
		cpu_stat->cpustat[CPUTIME_IDLE]		+= keydebug_get_idle_time(&kcpustat, cpu);
		cpu_stat->cpustat[CPUTIME_IOWAIT]	+= get_iowait_time(&kcpustat, cpu);
		cpu_stat->cpustat[CPUTIME_IRQ]		+= cpustat[CPUTIME_IRQ];
		cpu_stat->cpustat[CPUTIME_SOFTIRQ]	+= cpustat[CPUTIME_SOFTIRQ];
		cpu_stat->cpustat[CPUTIME_STEAL]	+= cpustat[CPUTIME_STEAL];
		cpu_stat->cpustat[CPUTIME_GUEST]	+= cpustat[CPUTIME_GUEST];
		cpu_stat->cpustat[CPUTIME_GUEST_NICE]	+= cpustat[CPUTIME_GUEST_NICE];
	}
#endif
}
